...
def calculate_rsi(series, period=14):
    ...
    # Użycie EMA zamiast średniej kroczącej
...
def calculate_stochastic(high, low, close, k_window=14, d_window=3):
    ...
    denominator = denominator.replace(0, 1)  # Zastępuje 0 przez 1, aby uniknąć dzielenia przez zero
    
    stochastic_k = 100 * ((close - lowest_low) / denominator)
    stochastic_d = stochastic_k.rolling(window=d_window, min_periods=1).mean()
    return stochastic_k, stochastic_d

# Poprawiona funkcja do obliczania ATR z użyciem pd.concat
def calculate_atr(high, low, close, period=14):
    high_low = high - low
    high_close_prev = (high - close.shift()).abs()
    low_close_prev = (low - close.shift()).abs()
    
    # Połączenie serii w ramkę danych
    tr_df = pd.concat([high_low, high_close_prev, low_close_prev], axis=1)
    
    # Obliczenie maksymalnej wartości w każdym wierszu
    tr = tr_df.max(axis=1)
    
    # Obliczenie ATR jako średniej kroczącej TR
...
def analyze_stock(ticker, indicators_weights):
    ...
    # Najnowsze wartości (rzutowanie na float)
    ...
    # Debugging: Wyświetlenie typów i wartości
    st.write(f"Ticker: {ticker}")
    st.write(f"latest_close: {latest_close} (type: {type(latest_close)})")
    st.write(f"latest_sma50: {latest_sma50} (type: {type(latest_sma50)})")
    st.write(f"latest_sma200: {latest_sma200} (type: {type(latest_sma200)})")
